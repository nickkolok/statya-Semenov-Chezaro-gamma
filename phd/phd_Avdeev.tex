\documentclass[12pt,a4paper,openbib]{report}
\usepackage{amsmath}
\usepackage[utf8]{inputenc}
\usepackage[english,russian]{babel}
\usepackage{amsfonts,amssymb}
\usepackage{latexsym}
\usepackage{euscript}
\usepackage{enumerate}
\usepackage{graphics}
\usepackage[dvips]{graphicx}
\usepackage{geometry}
\usepackage{wrapfig}
\usepackage[colorlinks=true,allcolors=black]{hyperref}
\usepackage{bbm}
\usepackage{enumitem}
\usepackage{mathrsfs}
\usepackage{fancyhdr}


% https://tex.stackexchange.com/questions/634509/show-hide-thumbnail-sidebar-by-default-in-pdf
\hypersetup{pdfpagemode=UseNone}


\righthyphenmin=2

%\usepackage[14pt]{extsizes}

\geometry{left=2.5cm}% левое поле
\geometry{right=1cm}% правое поле
\geometry{top=2cm}% верхнее поле
\geometry{bottom=2cm}% нижнее поле

\setcounter{tocdepth}{1}

\renewcommand{\baselinestretch}{1.3}

\renewcommand{\le}{\leqslant}
\renewcommand{\ge}{\geqslant}
\renewcommand{\leq}{\leqslant}
\renewcommand{\geq}{\geqslant} % И делись оно всё нулём!

\renewcommand{\varlimsup}{\limsup}
\renewcommand{\varliminf}{\liminf} % ... по самую асимптоту!



\DeclareMathOperator{\ext}{ext}
\DeclareMathOperator{\mes}{mes}
\DeclareMathOperator{\supp}{supp}
\DeclareMathOperator{\conv}{conv}
\DeclareMathOperator{\diam}{diam}
\DeclareMathOperator{\Lin}{Lin}

\newcommand{\N}{\ensuremath{\mathbb{N}}}
\newcommand{\Q}{\ensuremath{\mathbb{Q}}}
\newcommand{\R}{\ensuremath{\mathbb{R}}}
\newcommand{\B}{\ensuremath{\mathfrak{B}}}
\newcommand{\Iac}{\mathcal{I}(ac_0)}
\newcommand{\Dac}{\mathcal{D}(ac_0)}
\newcommand{\one}{\ensuremath{\mathbbm 1}}


\newcommand{\longcomment}[1]{}

\usepackage[backend=biber,style=gost-numeric,sorting=none,maxnames=4,movenames=false]{biblatex}
\addbibresource{../bib/Semenov.bib}
\addbibresource{../bib/my.bib}
\addbibresource{../bib/ext.bib}
\addbibresource{../bib/classic.bib}
\addbibresource{../bib/Damerau-Levenstein.bib}
\addbibresource{../bib/general_monographies.bib}
\addbibresource{../bib/Bibliography_from_Usachev.bib}

\input{../bib/ext.hyphens.bib}

\usepackage{amsthm}
\theoremstyle{definition}
\newtheorem{lemma}{Лемма}[section]
\newtheorem{theorem}[lemma]{Теорема}
\newtheorem{example}[lemma]{Пример}
\newtheorem{property}[lemma]{Свойство}
\newtheorem{remark}[lemma]{Замечание}
\newtheorem{definition}[lemma]{Определение}
\newtheorem{proposition}[lemma]{Утверждение}
\newtheorem{corollary}[lemma]{Следствие}

\newtheorem{hhypothesis}[lemma]{Гипотеза}


\newcommand\hypotlist{ }
\newcounter{hypcount}

\makeatletter
\usepackage{environ}
\NewEnviron{hypothesis}{%

	\edef\curlabel{hhypothesis\thehypcount}
    \begin{hhypothesis}
		\label{\curlabel}
		\BODY%
    \end{hhypothesis}
	\edef\curref{\noexpand\ref{\curlabel}}

	\expandafter\g@addto@macro\expandafter\hypotlist\expandafter
	{\paragraph{Гипотеза\!\!\!}}


	\expandafter\g@addto@macro\expandafter\hypotlist\expandafter
	{\expandafter\textbf\expandafter{\curref}}

	\expandafter\g@addto@macro\expandafter\hypotlist\expandafter
	{\textbf{.}~~}

	\expandafter\g@addto@macro\expandafter\hypotlist\expandafter
	{\BODY}

	\addtocounter{hypcount}{1}
}
\makeatother

%For the introduction
\newcommand\reflecttheorem[1]{\paragraph{Теорема \ref{#1}}}
\newcommand\reflectlemma[1]{\paragraph{Лемма \ref{#1}}}
\newcommand\reflectcorollary[1]{\paragraph{Следствие \ref{#1}}}
\newcommand\reflectdefinition[1]{\paragraph{Определение \ref{#1}}}
\newcommand\reflectproposition[1]{\paragraph{Утверждение \ref{#1}}}

%For the main text, we do not make the difference between
%self-citation and external citation
%(for autoref we do really do!)
\def\selfcite{\cite}

%Only referenced equations are numbered
\usepackage{mathtools}
\mathtoolsset{showonlyrefs}

%\mathtoolsset{showonlyrefs=false}
% (an equation/multline to be force-numbered)
%\mathtoolsset{showonlyrefs=true}

% https://superuser.com/questions/517025/how-can-i-append-two-pdfs-that-have-links
\usepackage{pdfpages}% http://ctan.org/pkg/pdfpages


% Центрируем заголовки, как того требует ГОСТ
\makeatletter
\patchcmd {\@makechapterhead}{\Huge \bfseries}{\Large \bfseries \centering }{}{\ddy}
\patchcmd {\@makechapterhead}{\huge \bfseries}{\large \bfseries \centering }{}{\ddy}
\patchcmd{\@makeschapterhead}{\Huge \bfseries}{\Large \bfseries \centering }{}{\ddy}

\patchcmd{\section}         {\Large \bfseries}{\large \bfseries \centering }{}{\ddy}
\patchcmd{\subsection}      {\large \bfseries}{       \bfseries \centering }{}{\ddy}
\makeatother

\begin{document}
\clubpenalty=10000
\widowpenalty=10000
\includepdf{title_HSE.pdf}



% Page numbering on the top
% redefine the plain pagestyle
\fancypagestyle{plain}{%
	\fancyhf{} % clear all header and footer fields
	\fancyhead[C]{\thepage} % except the center
	\renewcommand{\headrulewidth}{0pt}
}
\fancypagestyle{default}{%
	\fancyhf{} % clear all header and footer fields
	\fancyhead[C]{\thepage} % except the center
	\renewcommand{\headrulewidth}{0pt}
}

\pagestyle{plain}



\setcounter{page}{2}
\tableofcontents

\chapter*{Введение}
\addcontentsline{toc}{chapter}{Введение}

\section*{Общая характеристика работы}
\addcontentsline{toc}{section}{Общая характеристика работы}
\input{introduction_common_phd.tex}

\section*{Основные математические объекты и их обозначения}
\addcontentsline{toc}{section}{Основные математические объекты и их обозначения}
\input{intro_designations.tex}

\section*{Краткое содержание работы}
\addcontentsline{toc}{section}{Краткое содержание работы}
\input{introduction.tex}

\chapter{Последовательности, почти сходящиеся к нулю}

	\input{../episodes_phd/ac_intro.tex}

	\section{О декомпозиции ограниченной последовательности}
	\input{../episodes_phd/decomposition.tex}

	\section{Эквивалентность Дамерау--Левенштейна}
	\input{../episodes_phd/equiv_Damerau.tex}

	\section{Модифицированный критерий Лоренца}
	\input{../episodes_phd/criterion_ac0_quantors.tex}

	\section{О почти сходимости к нулю последовательности из нулей и единиц}
	\input{../episodes_phd/criterion_ac0_0_1_s.tex}
	\input{../episodes_phd/remark_C_j.tex}
	\input{../episodes_phd/sequence_M_j_limit.tex}

	\section{Пороговый критерий почти сходимости к нулю неотрицательной последовательности}
	\input{../episodes_phd/ac0_criterion_lambda.tex}

	\section{Пространство $ac_0$ и $\alpha$--функция --- исключён}
	%\input{../episodes_phd/alpha_ac0_c0.tex}

	\section{$\alpha$--функция на пространстве $ac$ и расстояние до пространства $c$}
	\input{../episodes_phd/alpha_ac_distance.tex}

	\section{Усиленная теорема Коннора}
	\input{../episodes_phd/Connor_generalized.tex}

	\section{О мере одного множества}
	\input{../episodes_phd/ac_W_measure.tex}

	\section{Пространство $ac_0$ и возведение в степень}
	\input{../episodes_phd/ac0_power.tex}


\chapter{$\alpha$--функция как асимптотическая характеристика ограниченной последовательности}

	\input{../episodes_phd/alpha_x_intro.tex}
	\section{Определение и элементарные свойства $\alpha$--функции --- редуцирован до вступления к главе}

	\section{$\alpha$--функция и оператор сдвига $T$}
	\input{../episodes_phd/alpha_Tx_leq_alpha_x.tex}

	\section{О характере сходимости последовательности $\alpha(T^n x)/ \alpha(x)$}
	\input{../episodes_phd/alpha_Tx_frac_alpha_x.tex}

	\section{О множествах $\{x: \alpha(T^n x) = \alpha(x)\}$}
	\input{../episodes_phd/alpha_Tx_equiv_alpha_x.tex}

	\section{$\alpha$--функция и семейство операторов $\sigma_n$}
	\input{../episodes_phd/alpha_sigma_n.tex}

	\section{$\alpha$--функция и семейство операторов $\sigma_{1/n}$}
	\input{../episodes_phd/alpha_sigma_1_n.tex}

	\section{$\alpha$--функция и оператор Чезаро $C$}
	\input{../episodes_phd/alpha_Cx.tex}

	\section{$\alpha$--функция и оператор суперпозиции}
	\input{../episodes_phd/alpha_xy.tex}

	\section{Функционалы $\alpha^*$ и $\alpha_*$ --- исключён}
	%\input{../episodes_phd/alpha_star.tex}

	\section{Пространство $\{x: \alpha(x) = 0\}$ --- сокращён и включен в состав следующего}

	\section{О недополняемости некоторых подпространств}
	\input{../episodes_phd/alpha_x_equiv_null_space.tex}
	\input{../episodes_phd/noncomplementarity.tex}

\chapter{Банаховы пределы, инвариантные относительно некоторых операторов}

	\input{../episodes_phd/invariant_BL_intro.tex}

	\section{Предварительные примеры инвариантности банаховых пределов}
	\input{../episodes_phd/example_finite_dim_kernel.tex}
	\input{../episodes_phd/example_infinite_dim_kernel.tex}
	\input{../episodes_phd/linear_op_equiv_ac0.tex}
	\input{../episodes_phd/cardinality_operators_invariant.tex}

	\section{Оператор с бесконечномерным ядром, относительно которого инвариантен любой банахов предел --- объединён с предыдущим}
	\section{О классах линейных операторов, для которых множества инвариантных банаховых пределов совпадают --- объединён с предыдущим}
	\section{Мощность множества линейных операторов, относительно которых инвариантен банахов предел --- объединён с предыдущим}

	\section{Банаховы пределы, инвариантные относительно операторов~$\sigma_{1/n}$ --- сокращён и включен в состав следующего}

	\section{Операторы $\sigma_{1/n}$ и $\tilde\sigma_k$}
	\input{../episodes_phd/invariant_sigma_1_n.tex}
	\input{../episodes_phd/operators_sigma_k.tex}

	\section{Мера прообраза числа при инвариантности банахова предела относительно оператора Чезаро --- исключён}
	%\input{../episodes_phd/measure_Chesaro_invariant.tex}


	\section{Классы линейных операторов по инвариантности относительно банаховых пределов}
	\input{../episodes_phd/operators_classification.tex}

	\section{Пример полуэберлейнового, но не эберлейнового оператора}
	\input{../episodes_phd/amiable_but_not_Eberlein_operator.tex}

	\section{Некоторые примеры В-регулярных и не В-регулярных операторов}
	\input{../episodes_phd/operator_that_excludes_2power_blocks.tex}
	%\section{Пример В-регулярного оператора неравномерного повторения}
	\input{../episodes_phd/sigma_triangle_operator.tex}

	\input{../episodes_phd/Eberlein_but_not_B-regular_operator.tex}

	\section{Пример эберлейнового не В-регулярного оператора --- сокращён и включен в предыдущий}


	\section{Обратная задача об инвариантности и порождённый оператор}
	\input{../episodes_phd/reverse_invariance.tex}

	\section{Мультипорождённые операторы}
	\input{../episodes_phd/multigenerated_operators.tex}

	\section{О функциональной характеристике банахова предела}
	\input{../episodes_phd/signature_gamma.tex}


\chapter{Функционалы Сачестона и линейные оболочки}

	\input{../episodes_phd/linhulls_intro.tex}

	\section{Вспомогательные построения}
	\input{../episodes_phd/linhulls_aux.tex}

	\section{Разделяющее множество нулевой меры}
	\input{../episodes_phd/linhulls_null_measure.tex}

	\section{Линейные оболочки множеств, определяемых функционалами Сачестона}
	\input{../episodes_phd/linhulls_Sucheston.tex}

	\section{О хаусдорфовой размерности одного класса множеств}
	\input{../episodes_phd/Omega_Hausdorf.tex}

	\section{О существовании разделяющих множеств малой хаусдорфовой размерности}
	\input{../episodes_phd/Omega_Hausdorf_existance.tex}


\chapter{Функционалы Сачестона и мультипликативные свойства носителя последовательности}

	\input{../episodes_phd/factors_intro.tex}

	\section{Конечное число множителей}
	\input{../episodes_phd/factors_finite.tex}

	\section{Бесконечное число множителей и \\  верхний функционал Сачестона}
	\input{../episodes_phd/factors_infinite_p.tex}

	\section{Бесконечное количество множителей и \\ нижний функционал Сачестона}
	\input{../episodes_phd/factors_infinite_q.tex}




\chapter*{Заключение}
\addcontentsline{toc}{chapter}{Заключение}
\input{conclusion.tex}
\input{acknowledgements.tex}

%\section*{Список сформулированных гипотез}
%\addcontentsline{toc}{section}{Список сформулированных гипотез}

%\renewcommand\label[1]{}
%\hypotlist


\addcontentsline{toc}{chapter}{Список литературы}

\makeatletter
\ltx@iffilelater{biblatex-gost.def}{2017/02/01}%
{\toggletrue{bbx:gostbibliography}%
\renewcommand*{\revsdnamepunct}{\addcomma}}{}
\makeatother

\printbibliography{}

\end{document}
