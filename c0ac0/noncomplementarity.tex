...
\title{
	On non-complementarity of $c_0$ in $ac_0$ and some other subspaces of $\ell_\infty$
}
...
Alekno~\cite[Theorem 8]{alekhno2006propertiesII}
gave an elegant proof of the non-complementarity of the inclusion $ac_0\subset\ell_\infty$.
...
\begin{proof}
	...
	Set $I_n = \{i \in I\,:\,(Qx_i)_n \neq 0\}$,
	then $I = \bigcup\limits_{n\in\mathbb{N}} I_n$
	is uncountable.
	Thus, we can find $n$ such that $I_n$ is also uncountable
	(otherwise $I$ would be countable as a countable junction of countable sets).

	Now set $I_{n,k} = \{i \in I\,:\,|(Qx_i)n| \geq 1/k\}$,
	then $I_n = \bigcup\limits_{k\in\mathbb{N}} I_{n,k}$.
	Applying the same argument, one can easily obtain that the set $I_{n,k}$ is uncountable for some $k$.
	Let us pick such $I_{n,k}$ and proceed with it.

	Let us consider a finite set $J \subset I_{n,k}$ with $\#J>1$
	...
	Note, that $f\in c_0$ and, with $c_0 \subseteq \ker Q$ we have $Qf = 0$.
	Thus, $Qy = Qz$ и $\|Q(y)\| \leq \|Q\| \|z\| \leq \|Q\|$.
	Due to~\eqref{eq:non_complemented_sum_cardinality}, we obtain $\# J \leq \|Q\| k$ for every $J\subset I_{n,k}$.
...
\begin{proof}
	...
	But then $\alpha(P\chi_S) = \alpha(\chi_S) = 1$, that is $P\chi_S \notin A_0$,
	which contaricts that $P$ is a projection onto $A_0$.
\end{proof}
...
\begin{proof}
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	The family of sets $\{S_i\}_{i \in I}$
	...
	\begin{equation}
		...
		.
	\end{equation}
	The proof is completed with the same argument with Lemma~\ref{lem:c_0_not_complemented}.
\end{proof}
...
\begin{proof}
	...
	(тsuch $x \in ac_0 \setminus c_0$ exists even if $\chi_S \notin ac_0$).
	But then $Px\notin c_0$.
	which contaricts that $P$ is a projection onto $c_0$.
\end{proof}
