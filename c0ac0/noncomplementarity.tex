...
The following lemma is adapted from~\cite{mathSE_Phillips}
with minimal changes to fit $ac_0$.
...
\begin{proof}
	Notice first that for any subset $S \subset \mathbb{N}$
	there exists $x\in ac_0$ with $\supp x \subseteq S$.
...
%Now we are going to prove some features of $F$.
The following property of $F$ is obvious:
...
Now define linear operator $F:\ell_\infty \to \ell_\infty$:
\begin{multline*}
	M\omega=\left(
		0, 1\omega_1,
	...
	.
\end{multline*}
Note that due to Proposition~\ref{thm:alpha_F} we have $FM: \ell_\infty \to A_0$.
...
\begin{lemma}
	...
	\end{equation}
	and $x\in A_0\setminus c_0$ such that $\supp x \in S$.
\end{lemma}
...
\begin{proof}
	...
	Suppose the contrary to the claim of the lemma:
	\begin{equation}
