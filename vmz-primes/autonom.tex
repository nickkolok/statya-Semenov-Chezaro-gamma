...
Работа посвящена изучения почти сходящихся последовательностей из 0 и 1.
...
\begin{lemma}
	...
	Пусть $m \in \mathbb{N}$
	и последовательность $x=\{x_k\}$ определённа соотношением
	\begin{equation}
		x_k = \begin{cases}
...
\begin{proof}
	...
	Итак, нам удалось получить число $f_0\in A$ и счётное множество $F$, обладающее $P$-свойством
...
Из следствия~\ref{cor:ac0_primes_sum_ln_diverges} и теоремы~\ref{thm:ac0_primes_p_psi_A_prod} вытекает
\begin{corollary}
	Пусть $\varepsilon \in {[} 0; 1)$.
	Тогда существует бесконечное число таких счётных множеств простых чисел $A$, что $p(\psi(A))=\varepsilon$.
\end{corollary}
