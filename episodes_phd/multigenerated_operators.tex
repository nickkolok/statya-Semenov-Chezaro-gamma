...
\begin{definition}
	...
	что для любого $j\in \mathbb N$ последовательность $\{h_{jk}\}_{k=1}^\infty \in \ell_1$
	и для любого $x\in\ell_\infty$
	\begin{equation}
...
\begin{theorem}[{\cite[\S 6.3]{sedaev2009_doc_vgasu}}]
...
Из этого результата ясно, что банаховы пределы не принадлежат $\ell_1$;
как следствие,
мультипорождённые операторы по своему определению в некотором смысле противоположны матричным операторам.
Говоря точнее, мультипорождённый оператор не может иметь матричного представления.
...
\begin{example}
	...
	\end{multline}
	Тогда оператор $A$ является В-регулярным по лемме~\ref{thm:A_block_thrower_is_B-regular}.
