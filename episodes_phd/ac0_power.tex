...
Итак, в недавней статье Р.Е. Зволинского~\cite{zvol2022ac} доказаны два следующих факта:
\begin{theorem}
	[{\cite[теорема 3]{zvol2022ac}}]
	\label{thm:Zvol_pow_pos}
	Пусть $x \geqslant 0, x \in a c_0$ и $\lambda>0$, тогда $x^\lambda \in a c_0$.
\end{theorem}

\begin{theorem}
	[{\cite[следствие 2]{zvol2022ac}}]
...
Однако для нечётной степени доказать аналог утверждения~\ref{thm:ac0_pow_even} не удастся "--- это показывает пример~\ref{example:ac0_pow_signum_classic}, в котором $x^+\in ac_1$, $x^-\in ac_{-1}$.
...
\begin{equation}
	...
	.
\end{equation}
(Множества $ac_0^{(2n)}$ мы в рассмотрение не вводим, поскольку каждое из них совпадает с $\Iac$ в силу утверждения~\ref{thm:ac0_pow_even}.)
