...
\begin{equation*}
	...
	,
\end{equation*}
уже знакомое нам, например, по параграфу~\ref{sec:noncomplementarity}.
...
\end{equation}
\begin{equation}
	Y^a_b = \{x\in A_0 : p(x) = a, q(x) = b\}
	.
\end{equation}
Подготовим сначала лемму о константе.

\begin{lemma}
	\label{lem:const_Lin_alpha_0}
	Справедливо включение
	$\one\in \operatorname{Lin} Y^a_b \operatorname{Lin} X^a_b$.
\end{lemma}

\begin{proof}
	Правое включение следует из того, что $Y^a_b \subset X^a_b$.
...
\begin{theorem}
	...
	\\
	(i) $\operatorname{Lin} Y^a_b = A_0$;
	\\
	(ii) $\operatorname{Lin} X^a_b = \ell_\infty$.
\end{theorem}
...
\begin{proof}
	...
	Тогда $x\in ac $ и потому, согласно теореме~\ref{thm:rho_x_c_leq_alpha_t_s_x_united}, $x\in c$
	и $x=k\cdot \one + y$, где $k\in\R$ и $y\in c_0$.
	В силу леммы~\ref{lem:const_Lin_alpha_0} достаточно показать, что $y\in\operatorname{Lin} Y^a_b$.
	Действительно,
	выберем произвольный $u \in Y^a_b$.
	Тогда
	\begin{equation}
		p(u+y) = p(u)
		,
		...
		,
	\end{equation}
	поэтому $u+y\in Y^a_b$ и, очевидно, $y=(u+y)-u\in\operatorname{Lin} Y^a_b$.
	...
	Действительно, в силу леммы~\ref{lem:const_Lin_alpha_0}~и выполнено
	...
	Снова, как и в доказательстве пункта (i),
	положим
	\begin{equation}
		y=k\cdot x + C\cdot\one,
		\quad\mbox{где}\quad
		...
		.
	\end{equation}
	Дальнейшее доказательство переносится из пункта (i) дословно.
\end{proof}
