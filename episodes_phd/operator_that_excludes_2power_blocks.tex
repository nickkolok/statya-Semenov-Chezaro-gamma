...
\begin{proof}
	Заметим, что $BA\in\B$ и $BA\sigma_k\in\B$ в силу леммы~\ref{thm:A_block_thrower_is_B-regular}, и потому
	\begin{equation}
...
Говоря строже, выполнена
\begin{lemma}
	Для любых $B_1 \in \B(\sigma_{2^{2n-1}})$, $n\in\N$ и $B_2 \in \B$
	выполнено
	\begin{equation}
		\|B_1 - B_2 A\| \geq 1
		.
...
\begin{corollary}
	Для любого $n\in\N$ выполнено
	$\rho(\B(A),\B(\sigma_{2^{2n-1}}))\geq 1$.
\end{corollary}
