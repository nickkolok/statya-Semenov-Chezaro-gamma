...
из утверждения~\ref{thm:ac0_primes_p_psi_A_prod} вытекает
\begin{corollary}
...
\begin{proof}
	По утверждению~\ref{thm:ac0_primes_p_psi_A_prod} условие~\eqref{eq:prod_causes_q} влечёт равенство $q(\chi\mathscr{M}A)=1$.
	По теореме~\ref{thm:p_x_infinite_multiples} $p(\chi\mathscr{M}A)=1$,
