...
\begin{theorem}
	...
	Для каждого $B\in \B$ существует такой компактный оператор $G_B:\ell_\infty \to \ell_\infty$,
	что $\B(G_B) = \{B\}$.
\end{theorem}
...
\begin{proof}
	...
	Покажем теперь, что определённый таким образом оператор компактен.
...
\end{definition}
	...
	поскольку $G_B^* \B = \{B\}$.
	Говоря о взаимодействии двух различных порождённых операторов,
...
\begin{example}
	...
	Напомним, что $\B(\sigma_2) = \B(\sigma_{1/2})$ в силу утверждения~\ref{thm:B_sigma_n_eq_B_sigma_1_n}, где
	\begin{equation}
...
В теореме~\ref{thm:generated_operator_G_B} мы построили компактный существенно эберлейнов оператор.
